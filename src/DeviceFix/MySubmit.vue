...
export default {
  ...
  methods: {
    ...
    refresh() {
      ...
      that.taskData = [];
      let url =
        "http://47.102.214.37:8080/issue/reporter?page=0&size=" +
        that.page_size;
    ...
    reverterror(index) {
      ...
      this.$confirm("确定撤销此报修申请吗？", "提示", {
        ...
        let url =
          "http://47.102.214.37:8080/issue/revert/" +
          that.taskData[index].errorid;
        axios
          .post(url)
          .then(() => {
            that.$message({
    ...
    handleSizeChange(val) {
      ...
      that.page_size = val;
      let url =
        "http://47.102.214.37:8080/issue/reporter?page=0&size=" + that.size;
      console.log(url);
      axios.get(url).then((res) => {
        console.log(res.data);
        that.total = res.data.totalElements;
    ...
    handleCurrentChange(val) {
      ...
      console.log(val);
      let url =
        "http://47.102.214.37:8080/issue/reporter?page=" +
        (that.page - 1) +
        "&size=" +
        that.size;
      console.log(url);
