...
export default {
  ...
  methods: {
    ...
    editcontent() {
      ...
      this.contentid = this.TaskInfo.content[this.contentindex].id;
    ...
    submittask() {
      ...
      if (
        ...
        axios.get(url).then((res) => {
          ...
          let obj = {};
          let content = [];
          for (let i = 0; i < that.TaskInfo.content.length; i++) {
            content.push({
              id: that.TaskInfo.content[i].id,
              detail: that.TaskInfo.content[i].detail,
              title: that.TaskInfo.content[i].title,
            });
          }
